...
const roadmaps: AdminRoadmap[] = [
  ...
  {
    id: 8, name: "Roadmap Squad Cloud 2026", slug: "roadmap-squad-cloud-2026", description: "",
    ownerId: 2, ownerName: "Eduarda", ownerEmail: "eduarda@x.com",
...
describe("AdminRoadmaps", () => {
  ...
  it("lista todos os roadmaps com dono, e-mail e contagens", async () => {
    ...
    expect(screen.getByText("Roadmap Squad Cloud 2026")).toBeInTheDocument();
  });

  it("filtra por dono", async () => {
    setup();
    await waitFor(() => expect(screen.getByText("Roadmap VPS 2026")).toBeInTheDocument());
    fireEvent.change(screen.getByLabelText("filtrar roadmaps"), { target: { value: "eduarda" } });
  ...
  it("transfere o dono sem manter o anterior (caso de quem saiu da empresa)", async () => {
    transferRoadmapOwner.mockResolvedValue({ ...roadmaps[0], ownerId: 2, ownerName: "Eduarda", ownerEmail: "eduarda@x.com" });
    setup();
    await waitFor(() => expect(screen.getByText("Roadmap VPS 2026")).toBeInTheDocument());
    ...
    await waitFor(() => expect(screen.getAllByText("eduarda@x.com").length).toBeGreaterThan(0));
  });

  it("permite manter o dono anterior como colaborador", async () => {
    transferRoadmapOwner.mockResolvedValue({ ...roadmaps[0], ownerId: 1, ownerName: "Fagner", ownerEmail: "fagner@x.com" });
    setup();
    await waitFor(() => expect(screen.getByText("Roadmap VPS 2026")).toBeInTheDocument());
  ...
  it("mostra o erro do servidor quando o novo dono já tem roadmap homônimo", async () => {
    transferRoadmapOwner.mockRejectedValue(new Error('Eduarda já tem um roadmap chamado "Roadmap VPS 2026".'));
    setup();
    await waitFor(() => expect(screen.getByText("Roadmap VPS 2026")).toBeInTheDocument());
    fireEvent.click(screen.getAllByRole("button", { name: /Transferir dono/ })[0]);
    fireEvent.change(screen.getByLabelText("Novo dono"), { target: { value: "2" } });
